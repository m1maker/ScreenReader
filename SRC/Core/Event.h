...
public:
	...
	struct SHotkeyInfo final {
		...
		explicit constexpr SHotkeyInfo(
			const EKeycode& keycode = KEYCODE_NONE, const unsigned char& modifiers = MODIFIER_NONE)
	...
	[[nodiscard]] static inline auto GetModifierNames(const unsigned char& modifiers) -> std::string {
		...
		if (modifiers & MODIFIER_SHIFT)
			modifier_names.emplace_back("Shift");
		if (modifiers & MODIFIER_CTRL)
			modifier_names.emplace_back("Ctrl");
		if (modifiers & MODIFIER_ALT)
			modifier_names.emplace_back("Alt");
		if (modifiers & MODIFIER_SUPER)
			modifier_names.emplace_back("Super");
		if (modifiers & MODIFIER_CAPS_LOCK)
			modifier_names.emplace_back("Caps Lock");
		if (modifiers & MODIFIER_NUM_LOCK)
			modifier_names.emplace_back("Num Lock");
	...
	[[nodiscard]] static inline auto KeyComboToString(const EKeycode& keycode, const unsigned char& modifiers)
		-> std::string {
