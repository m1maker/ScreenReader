...
#define EnableBitwiseEnum(EnumType)                                                                                    \
	...
	[[nodiscard]] constexpr inline bool operator&(std::underlying_type_t<EnumType> lhs, EnumType rhs) noexcept {       \
		return (lhs & static_cast<std::underlying_type_t<EnumType>>(rhs)) != 0;                                        \
	}                                                                                                                  \
	inline constexpr EnumType& operator&=(EnumType& lhs, EnumType rhs) noexcept {                                      \
		lhs = lhs & rhs;                                                                                               \
		return lhs;                                                                                                    \
