...
[[nodiscard]] inline auto DecodeUtf8(const char* pointer) -> SUtf8Result {
	...
	if ((c & 0xF0) == 0xE0) {
		return { static_cast<uint32_t>(((c & 0x0F) << 12) | ((static_cast<unsigned char>(pointer[1]) & 0x3F) << 6) | (static_cast<unsigned char>(pointer[2]) & 0x3F)), 3 };
	}

	if ((c & 0xF8) == 0xF0) {
		return { static_cast<uint32_t>(((c & 0x07) << 18) | ((static_cast<unsigned char>(pointer[1]) & 0x3F) << 12) | ((static_cast<unsigned char>(pointer[2]) & 0x3F) << 6) | (static_cast<unsigned char>(pointer[3]) & 0x3F)), 4 };
	}
...
public:
	...
	[[nodiscard]] auto operator*() const -> uint32_t {
		return DecodeUtf8(m_pointer).code_point;
	}
