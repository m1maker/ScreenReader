...
enum class EObjectInterfaceMask : uint32_t {
	...
	[[nodiscard]] static auto GetObjectTypeName(EObjectType type, bool require_all = false) -> std::string_view;
	[[nodiscard]] static auto GetObjectStateNames(
		EObjectType type, unsigned long long states, bool require_all = false)
...
public:
    [[nodiscard]] auto GetCursor() const { return Impl().do_GetCursor(); }
    [[nodiscard]] auto GetText(int cursor, const ETextGranularity& granularity) const { return Impl().do_GetText(cursor, granularity); }
    [[nodiscard]] auto GetSelectedRanges() const { return Impl().do_GetSelectedRanges(); }
};

template <typename Derived>
class TSelectionProvider {
    BindStaticInterface(Derived)
public:
    [[nodiscard]] auto GetSelectedItems() const { return Impl().do_GetSelectedItems(); }
};
...
class TValueProvider {
    BindStaticInterface(Derived)
public:
    [[nodiscard]] auto GetMinValue() const { return Impl().do_GetMinValue(); }
...
public:
    ...
    [[nodiscard]] auto GetSupportedInterfaces() const noexcept -> uint32_t { return Impl().do_GetSupportedInterfaces(); }
    ...
    void UpdateCacheByEvent(CObjectEvent::EObjectEventType type) { Impl().do_UpdateCacheByEvent(type); }
};

template <class NativeHandle, struct ObjectData> class CObjectCache final {
...
public:
	...
	template<typename PlatformObject>
	[[nodiscard]] auto GetOrCreate(NativeHandle* native_handle) -> PlatformObject {
		...
		auto raw = pool.allocate(sizeof(ObjectData));
	...
	void Remove(NativeHandle* native_handle) {
		std::scoped_lock lock(m_mutex);
		auto it = m_cache.find(native_handle);
		if (it == m_cache.end()) [[unlikely]] return;
		it->second->~ObjectData();
		m_pool.deallocate(it->second, sizeof(ObjectData));
		m_cache.erase(it);
	}
